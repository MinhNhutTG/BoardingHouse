...
namespace QuanLyPhongTro.GUI.UC.Bill
{
    public partial class UC_Bill : UserControl
    {
        ...
        {
            List<string> listRoom = bllroom.GetRentedRooms();
            Thread thr = new Thread(() =>
            {
                foreach (string room in listRoom)
                {
                 
                    DTO.HistoryService hs = bllhistoryservice.FindHistoryServiceByIDRoomAndMonth(room, Handles.getPreviousSession());
                    DTO.Room r = bllroom.FindRoomByID(Convert.ToInt32(room));
                    DTO.Bill bill = new DTO.Bill();
                   
                    decimal price = bllcontract.FindContractByIDRoom(room);
                    ...
                    bill.TienNuoc = bill.SoNuoc * hs.GiaDien;
                    bill.PhiKhac = 0;
                    bill.GiaPhong = price;
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    List<DTO.Bill> listBill = bllBill.SearchBill(txtSearch.Text);
                    if (listBill.Count <=0 ) {
